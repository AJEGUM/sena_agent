...
def enviar_mensaje(texto):
    ...
    try:
        ...
        print("Error: Telegram tardo demasiado en responder")
        return False
...
def enviar_alerta_error(detalle_error):
    """
    Notifica cuando el agente fallo para que sepas
    que algo salio mal sin tener que revisar logs manualmente.
    """
    hora_actual = datetime.now().strftime("%d/%m/%Y %H:%M")

    mensaje = (
        f"*ALERTA - El agente SENA tuvo un problema*\n"
        f"{hora_actual}\n"
        f"{'─' * 30}\n\n"
        f"Error: {detalle_error}\n\n"
        f"Revisa los logs para mas detalles."
    )
